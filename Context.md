...
1. Давай определим домены и декомпозируем систему (DDD) - как можно разделить на ограниченные контексты?
   1. по ключевым данным системы
   2. по процессам
   3. регуляторные и лицензионные требования (GDPR, PCI итд)
   4. по юзер сценариям
   5. по связности с другими элементами, частоте обновлений компонент

2. Как внутри домена можно еще декомпозировать процессы? (+ если обратит на это внимание)
...
3. Сценарии работы с данными - Мы с контентом не работаем, но если у нас сильный перекос в сторону чтения данных происходит, то что что нам лучше сделать? 
...
> **Задача 1:** У нас 2 домена - заказы и delivery. Но для юзера это выглядит как единый процесс с единым статусом. Как нам можно синхронизировать состояние каждого процесса в каждом домене?
>- Нужен оркестратор (SAGA) или хореография. В чем их плюсы и минусы?
>- Где можно расположить логику оркестрации? (на клиенте если stateless, в домене orders, в отдельном сервисе если stateful)

>**Задача 2:** Юзера нужно уведомлять об изменении статуса по карте. Какие есть способы?
>- сам юзер обновляет статус
...
3. Составим примерный DTO JSON (нужно пошарить экран и блокнот)
...
1. Какой протокол или несколько выберем для нашей системы? **rest / rpc / grpc / graphql / soap**
...
>**Задача 1:** У нас есть контракт от джуна - надо поревьюить и найти несколько ошибок https://raw.githubusercontent.com/konkrr/hiring-tasks/main/card-delivery-openapi.yaml